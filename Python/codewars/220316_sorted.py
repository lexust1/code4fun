...
# when provided with a triplet, returns the index of the numerical element 
...
def gimme(input_array):
    idx_max_el = input_array.index(max(input_array))
    idx_min_el = input_array.index(min(input_array))
    idx = [el for el in range(3) if el != idx_max_el and el != idx_min_el]
    return idx
