...
def find_edges(graph, end1, end2):
    """
    Находит все рёбра в графе, которые лежат на кратчайшем пути между
    ...
    Returns:
        list of tuples: Список рёбер, которые лежат на кратчайшем пути,
            или пустой список, если путь между вершинами не существует.
    """
    ...
    edges_on_paths = []
    # Перебираем все вершины и их соседей в графе
    for vertex in graph:
        for neighbour in graph[vertex]:
            # Проверяем, удовлетворяет ли ребро (vertex1, vertex2)
            # критерию кратчайшего пути
            if dist_end1[vertex] + dist_end2[neighbour] + 1 == dist_end1[end2]:
                edges_on_paths.append((vertex, neighbour))
...
def main():
    ...
    print(find_edges(graph, "6", "8"))
    print(find_edges(graph, "11", "13"))
