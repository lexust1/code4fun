...
# Пусть даны две строки. Найти все вхождения второй строки в первую с
...
def KMP_search_all(main_string, substring):
    """
    Ищет ВСЕ!!! вхождения подстроки в строке, используя алгоритм
    ...
    Returns:
    List[int]: Список индексов, где начинаются вхождения подстроки в основной строке.
    """
    pi = compute_pi_function(substring)
    j = 0
    indices = []  # Список для хранения индексов начала вхождений подстроки
    ...
    for i in range(len(main_string)):
        ...
        if j == len(substring):
            indices.append(i - j + 1)  # Добавляем индекс начала вхождения подстроки
            j = pi[j - 1]  # Продолжаем поиск с текущей позиции
    return indices



def main():
    # result = KMP_search_all("abcxabcdabcdabcy", "abcdabcy")
    result = KMP_search_all("abcxabcdabcdabcy", "abc")
    print(result)
