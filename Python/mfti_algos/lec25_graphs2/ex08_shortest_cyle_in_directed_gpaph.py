...
# Вывод
# 1 6 7 3
# Как я понимаю, для направленных графов есть типовые алгоритмы типа,
# алгортима Дейкстры. Здесь я просто доработал алгоритм для поиска
# кратчаешего пути в ненаправленном графе.
    ...
    while queue:
        ...
        for neighbour in graph[vertex]:
            ...
            # мы нашли цикл.
                ...
                return predecessors, neighbour, vertex
    # Если нет циклов
    return False
...
def find_shortest_cycle(graph):
    ...
    for vertex in graph:
        ...
        # Если цикл не найден, пропускаем итерацию
            ...
            continue
        # Если найден цикл, реконструируем его (однако в ходе
        # реконструкции может оказаться, что это и не цикл).
        else:
            reconstructed_cycle = reconstruct_cycle(*candidate)
            # Если выясняется, что оказывается есть вершина без
            # предшественника, то это говорит о том, что это не цикл,
            # а ветвь, которая искажает критерий работы по длине.
            ...
            # Если все-таки цикл
                ...
                print(
                    f"Cycle when we start from vertex {vertex}: "
                    f"{reconstructed_cycle}"
                )
                # Обновляем кратчайший цикл, если найденный короче текущего
                if len(reconstructed_cycle) < cycle_length:
                    cycle_length = len(reconstructed_cycle)
                    cycle = reconstructed_cycle
    # Возвращаем цикл
    return cycle
...
def reconstruct_cycle(predecessors, end1, end2):
    ...
    while vertex != end1:
        ...
        # Если предшественника нет, то и цикла нет, т.к. мы не дошли
        # до начальной вершины. Возрващаем False, чтобы не пытался
        # определять длину этого "недоцикла".
        if vertex is None:
            return False
