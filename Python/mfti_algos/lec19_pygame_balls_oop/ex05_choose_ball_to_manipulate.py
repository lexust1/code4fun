# Упражнение №5
# При клике мышью по шарику, этот шарик выбирается для ручного
# управления с клавиатуры (как в предыдущей лабораторной)
import pygame
import random
import sys
    ...
    while True:
        ...
        for ball in balls:
            if ball == selected_ball:
                ball.apply_friction(dt)  # Применяем трение только к выбранному шарику
            ball.update_position(dt)  # Обновляем позицию для всех шариков
            ball.mirror_ball(width, height)  # Обрабатываем отражение от краев для всех шариков
            ball.render(screen)
        pygame.display.flip()
